...
object Loops {
  def main(args: Array[String]): Unit = {
    ...
    val kelime = "Scala"
    /*for(i <- 0 to kelime.length){ // Burada hata alacağız çünkü son sınır dahil
      println(kelime(i))
    }*/


    println("for döngüsü kelime örneği son sınır hariç")
    for(i <- 0 until kelime.length){ // Bu düzgün çalışır çünkü son sınır dahil değil
      println(kelime(i))
    }
